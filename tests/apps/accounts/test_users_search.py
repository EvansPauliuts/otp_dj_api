...
class TestUsersSearch:
    ...
    def test_search_username_list(self, api_client, user):
        response = api_client.get(self.api_endpoint + f'?search={user.username}', format='json')
    ...
    def test_search_username_empty(self, api_client, user):
        ...
        response = api_client.get(self.api_endpoint + f'?search={wrong_user_name}', format='json')

        assert len(response.data['results']) == 0

    def test_search_email_list(self, api_client, user):
        response = api_client.get(self.api_endpoint + f'?search={user.email}', format='json')
    ...
    def test_search_email_empty(self, api_client, user):
        ...
        response = api_client.get(self.api_endpoint + f'?search={wrong_email}', format='json')
